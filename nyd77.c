...
int *solucao(struct entrada *entradas, int m, int n_teams)
{
   ...
   for(j=0 ;j < m; j++) // 1 atrib + n*(n-1)/2 + 1 comp + n*(n-1)/2 atrib
    {
        ...
        if(entradas[j].y > entradas[j].w) // custo = 1 comp + 2 atrib + 2 somas + 2 sub 
        {
    ...
    for(j = 0; j < n_teams; j++) //1 atrib, n incrementos, n+1 comp
    {
        if(aux[j].cestastomadas) // custo 2
